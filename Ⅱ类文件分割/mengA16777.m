...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [1,1,width+5,m]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [width+8,6,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [3*width+4,1,y-2*width-8,x]));
    end
end

...
% 中值滤波
plate_img2 = medfilt2(plate_img1);
subplot(3,3,3);
imshow(plate_img2);
title('中值滤波后的图像');

% 直方图均衡化
plate_img2 = histeq(plate_img2);
subplot(3,3,4);
imshow(plate_img2);
title('直方图均衡化的图像');
subplot(3,3,5);
...
% %将图像锐化
% Ig2=double(plate_img2);
% w=fspecial('laplacian',0);
% g1=imfilter(plate_img2,w,'replicate');
% plate_img2=plate_img2-3*g1;
% subplot(3,3,6);
% imshow(plate_img2);
% title('锐化图');

%图像倒置
plate_img3 = imcomplement(plate_img2);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像倒置');

% 二值化处理
plate_img4 = im2bw(plate_img3, 0.7);
subplot(3,3,8);
imshow(plate_img4);
title('车牌二值图像');

% 中值滤波
plate_img4 = medfilt2(plate_img4);
subplot(3,3,9);
imshow(plate_img4);
title('中值滤波后的图像');
...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [6,10,width*0.47,m-20]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [0.5*width,25,n-15,m-13]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [width+8,8,y-width,x]));
    end
end
[m,n]=size(plate_img5);
plate_img5=my_imsplit(imcrop(plate_img5, [1,25,n-15,m-48]));
 ...
 % 分割出第六、七个字符
 [word6,word7]=getword(plate_img5);

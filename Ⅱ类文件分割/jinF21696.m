...
% 二值化处理
plate_img3 = im2bw(plate_img1, 0.7);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像');
...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [1,1,19+width,m]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [18+width,0,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [39+width,1,y-width-39,x]));
    end
end

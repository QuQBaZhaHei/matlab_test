...
%% 边缘提取
img4 = edge(img1, 'roberts', 0.1, 'both');
subplot(3,3,4);
imshow(img4);
title('roberts算子边缘检测');
...
%% 调整亮度
plate_img2=adapthisteq(plate_img1);
subplot(3,3,3);
imshow(plate_img2);
...
% 二值化处理
plate_img3 = im2bw(plate_img2, 0.65);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像');
...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [1,15,40+width,m-20]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [40+width,14,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [90+width,1,y-width-39,x]));
    end
end

[m,n]=size(plate_img5)
plate_img5=my_imsplit(imcrop(plate_img5,[0,10,n,m-10]));

...
%% 边缘提取
img4 = edge(img1, 'sobel', 0.26, 'both');
subplot(3,3,4);
imshow(img4);
title('roberts算子边缘检测');
%% 图像腐蚀
...
% 转换成灰度图像
[m,n]=size(plate_img);
plate_img1 = my_imsplit(imcrop(plate_img, [18,105,n-300,m-113]));
plate_img1 = rgb2gray(plate_img1);    % RGB图像转灰度图像
figure;
subplot(3, 3, 1);
imshow(plate_img1);
title('灰度图像');
...
[m,n]=size(plate_img5)
%plate_img5 = my_imsplit(imcrop(plate_img5, [10,33,n-20,m-48]));
...
while flag == 0
    ...
    if width < y1
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [width*1.2,3,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [width*3,1,y-width,x]));
    end
end

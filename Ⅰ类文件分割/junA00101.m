...
%% 边缘提取
img4 = edge(img1, 'roberts', 0.2, 'both');
subplot(3,3,4);
imshow(img4);
title('roberts算子边缘检测');
...
%% 二值化处理
plate_img3 = im2bw(plate_img1, 0.35);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像');
...
[m,n]=size(plate_img5)
plate_img5 = my_imsplit(imcrop(plate_img5, [2,5,n,m-5]));
...
while flag == 0
    ...
    if width < y1
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [width+3,3,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [2*width+9,1,y-2*width-9,x]));
    end
end

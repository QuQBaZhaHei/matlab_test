...
%% 边缘提取
img4 = edge(img1, 'roberts', 0.05, 'both');
subplot(3,3,4);
imshow(img4);
title('roberts算子边缘检测');
...
% 二值化处理
plate_img3 = im2bw(plate_img1, 0.6);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像');
...
[m,n]=size(plate_img5)
plate_img5 = my_imsplit(imcrop(plate_img5, [2,18,n,m-18]));
...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [0,1,width+3,m]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [width+10,1,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [2*width+40,1,y-2*width-15,x]));
    end
end

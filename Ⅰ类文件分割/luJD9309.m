...
%% 边缘提取
img4 = edge(img1, 'roberts', 0.11, 'both');
subplot(3,3,4);
imshow(img4);
title('roberts算子边缘检测');
...
% 二值化处理
plate_img3 = im2bw(plate_img1, 0.45);
subplot(3,3,7);
imshow(plate_img3);
title('车牌二值图像');
...
% plate_img4 = imcomplement(plate_img3);
% subplot(3,3,8);
% imshow(plate_img4);
% title('车牌二值图像倒置');
...
[m,n]=size(plate_img5)
plate_img5 = my_imsplit(imcrop(plate_img5, [2,18,n,m-25]));
...
while flag == 0
    ...
    if width < y1
        ...
        temp = my_imsplit(imcrop(plate_img5, [0,1,width+100,m]));
        [m, n] = size(temp);
        all = sum(sum(temp));
        ...
        plate_img5(:, [1:1]) = 0;
        plate_img6 = my_imsplit(imcrop(plate_img5, [width+120,1,n,m]));
        plate_img5 = my_imsplit(imcrop(plate_img5, [width+250,1,y-width-250,x]));
    end
end
